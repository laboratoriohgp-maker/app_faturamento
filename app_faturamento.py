...
# ---------------------- Auto-detect Columns ----------------------
mv_patient_col = find_col(mv, ['paciente', 'nome', 'patient'])
mv_exam_col = find_col(mv, ['exame', 'proced', 'procedimento', 'exam'])
mv_id_col = find_col(mv, ['atendimento', 'id_atend', 'numero_atendimento', 'num_atend', 'id'])
mv_time_col = detect_datetime_col(mv)
mv_value_col = find_col(mv, ['valor', 'preco', 'price', 'value'])
mv_plan_col = find_col(mv, ['convenio', 'plano', 'plan', 'insurance'])

lab_patient_col = find_col(lab, ['paciente', 'nome', 'patient'])
lab_exam_col = find_col(lab, ['exame', 'proced', 'procedimento', 'exam'])
lab_id_col = find_col(lab, ['atendimento', 'id_atend', 'numero_atendimento', 'id'])
lab_time_col = detect_datetime_col(lab)
lab_value_col = find_col(lab, ['valor', 'preco', 'price', 'value'])

# Validação
if mv_patient_col is None or mv_exam_col is None:
    st.error("❌ Colunas essenciais não encontradas na planilha MV (paciente/exame)")
    st.info("Colunas disponíveis: " + ", ".join(mv.columns))
    st.stop()

if lab_patient_col is None or lab_exam_col is None:
    st.error("❌ Colunas essenciais não encontradas na planilha LAB (paciente/exame)")
    st.info("Colunas disponíveis: " + ", ".join(lab.columns))
    st.stop()
...
if mv_value_col:
    mv['valor_mv'] = pd.to_numeric(mv[mv_value_col], errors='coerce')
if mv_plan_col:
    mv['convenio_norm'] = mv[mv_plan_col].apply(normalize_text)
...
if lab_value_col:
    lab['valor_lab'] = pd.to_numeric(lab[lab_value_col], errors='coerce')
